...
from globals import ID2FewerMidas as ID2Midas, ID2Entity, EntityTargets2ID as Entity2ID

class Inference:

    def __init__(self, vectorizer, ranker,
                 midas_clf, entity_clf,
                 labels2response_ids, responses,
                 encoded_responses):
    ...
    def infer_utterance(self, sample) -> dict:
        ...
        candidates = self.encoded_responses[candidate_ids, :]
        context = " ".join([" ".join(ut) for ut in sample['previous_text']])

        scores = self.ranker(context, candidates, batch_size=32)
        candidate_pos = np.argmax(scores)
        candidate_id = candidate_ids[candidate_pos]
        # print out
        context = " __eou__ ".join([" ".join(ut) for ut in sample['previous_text']])
        text = self.responses[candidate_id]
