...
class MidasDataset(Sequence):
    ...
    def __init__(
        self, data: list, vectorizer, batch_size: int = 32, shuffle: bool = False):
        self.data = data
    ...
    def __getitem__(self, idx: int):
        ...
        for i, sample in enumerate(batch):
            x_batch[i, :] = self.vectorizer.context_vector(
                sample['previous_text'], sample['midas_vectors'])
...
class MidasVectorizer:
    ...
    def __init__(
        self, text_vectorizer, midas2id: dict, context_len: int=3, embed_dim: int = 512):
    ...
    def __get_context_vec(
        self, embedding: np.ndarray, midas_vec: np.ndarray) -> np.ndarray:
        """ 
